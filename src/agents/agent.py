...
def build_agent(ctx=None):
    ...
    def _invoke_research_flow(topic: str, direction: str = "",
        ...
        try:
            ...
            if result.get("ppt_prompt"):
                output_parts.append("\n---\n\n")
                output_parts.append("## 📊 PPT 提示词\n\n")
                output_parts.append(result["ppt_prompt"])

            if result.get("references"):
                output_parts.append("\n---\n\n")
                output_parts.append("## 📚 附录：信息来源\n\n")
                output_parts.append(result["references"])
    ...
    # 收集所有工具
    all_tools = [run_research_flow, research_search, deep_search]
