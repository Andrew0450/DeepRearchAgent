...
3. **竞争格局**：
   ...
   - 市场集中度（CR3/CR5等指标）
   - 中外阵营对比（国际 vs 国内各自的优劣势）
   - 注：具体产品的功能/参数/价格横向对比见竞品分析方向（D），此章节不做产品横向对比
4. **关键玩家**：每个梯队2-3家代表性企业，简介其核心优势、主要产品、市场定位
5. **驱动因素**（技术/需求/资本）。注：政策驱动见政策趋势方向（E），此章节不展开
6. **行业痛点与挑战**：
...
_SEARCH_QUERIES_B1 = [
    '{primary} 市场规模 产业链 头部企业 市场份额 集中度',
    '{primary} 行业分析 竞争格局 梯队 研报',
...
### 第三层：宏观环境
1. **政策一句话**：提及相关政策名称和简要方向（详细政策分析见政策趋势方向E）。注：不做政策解读
2. **技术趋势**：技术发展方向、新兴技术威胁
3. **国际市场**：全球市场格局、进出口情况、国际竞争
4. **替代品威胁**：是否存在替代方案？替代品的优劣势？
注：本章节为背景参考，不做展开，核心篇幅留给痛点分析
...
## 【禁止】本方向不做以下内容
- ❌ 不写市场规模数据（那是行业调研方向B1/产品调研方向B2的工作）
- ❌ 不写产业链分析
- ❌ 不写竞品对比
- ❌ 不做市场预测
...
### 模板B1（行业驱动型）
行业概述 → 市场规模与趋势（含历年数据表格） → 产业链拆解 → 竞争格局（梯队/份额/集中度/中外对比，不做产品横向对比） → 关键玩家（每个梯队2-3家代表性企业） → 驱动因素（技术/需求/资本，不含政策） → 行业痛点与挑战（重点章节） → 风险与挑战 → 趋势判断
注：政策驱动详见政策趋势方向（E），竞品详细功能/参数对比详见竞品分析方向（D）

### 模板B2（产品驱动型）
产品与行业定义 → 产品直接市场（含历年数据表格） → 应用场景市场 → 产业链全景 → 竞争格局（一句话） → 宏观环境（政策一句+技术趋势，不做展开） → 行业痛点与挑战（重点章节：产品现存问题+用户抱怨+技术瓶颈） → 风险与挑战 → 趋势判断
注：详细政策分析见政策趋势方向（E），详细竞品对比见竞品分析方向（D）
